...
import { RefreshCw, Layers, Code2 } from 'lucide-react';
...
export const CodeRefactorer = () => {
  ...
  const refactorTypes = [
    'Extract Functions', 'Simplify Logic', 'Improve Naming', 'Remove Duplication', 'Design Patterns', 'General Cleanup'
  ];
  ...
  const handleRefactor = async () => {
    ...
    const prompt = `Refactor this ${language} code to improve maintainability and readability:

Code:
${code}

${refactorType ? `Focus on: ${refactorType}` : 'General refactoring'}
...
7. Maintain exact same functionality
8. Add helpful comments where needed

Provide clean, well-structured code with explanations of changes made.`;

    try {
      const result = await aiService.processPrompt(prompt, 'refactoring', 'Both');
    ...
    <div className="flex h-full">
      <div className="w-1/2 border-r border-green-500/30 flex flex-col bg-gray-950/50">
        <div className="p-6 border-b border-green-500/20 bg-gradient-to-r from-gray-900 to-gray-800">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-300 font-mono">Improve code structure and maintainability</p>
            </div>
          </div>
          <Badge className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white border-0 font-mono">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-green-400 mb-2 font-mono">REFACTOR_TYPE</label>
            <Select value={refactorType} onValueChange={setRefactorType}>
              <SelectTrigger className="bg-gray-900/80 border-green-500/30 text-white font-mono">
                <SelectValue placeholder="Select refactoring focus (optional)..." />
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-green-500/30">
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-mono"
          >
            {isRefactoring ? (
